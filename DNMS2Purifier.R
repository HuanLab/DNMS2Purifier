...
target_extraction <- FALSE # TRUE:  Purification will be performed on target metabolic features
...
larger_index <- which(tb$frag_mz > tb$pre_mz + 0.02)
tb$prediction[larger_index] <- 0
precursor_index <- which(abs(tb$frag_mz - tb$pre_mz) <= 0.01)
tb$prediction[precursor_index] <- 1
outputcol <- 1:6
...
#############  make improvement to remove those fragment whose mz larger than precursor 0.02############
...
record <- record[-1,]
exclude_index <- which( factor_tb_NA$frag_mz > factor_tb_NA$pre_mz + 0.02)
naFragment <- factor_tb_NA[-exclude_index,]
